...
/**
 * Base class that represents a row from the 'product_image' table.
 *
 * 
 *
...
abstract class ProductImage implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\ProductImageTableMap';
    ...
    /**
     * Initializes internal state of Base\ProductImage object.
     */
    ...
    public function setProductId($v)
    {
        ...
        if ($this->product_id !== $v) {
            $this->product_id = $v;
            $this->modifiedColumns[ProductImageTableMap::COL_PRODUCT_ID] = true;
        }
    ...
    public function setImageId($v)
    {
        ...
        if ($this->image_id !== $v) {
            $this->image_id = $v;
            $this->modifiedColumns[ProductImageTableMap::COL_IMAGE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ProductImageTableMap::translateFieldName('ProductId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->product_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ProductImageTableMap::translateFieldName('ImageId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = ProductImageTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ProductImageTableMap::COL_PRODUCT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'product_id';
        }
        if ($this->isColumnModified(ProductImageTableMap::COL_IMAGE_ID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ProductImageTableMap::COL_PRODUCT_ID)) {
            $criteria->add(ProductImageTableMap::COL_PRODUCT_ID, $this->product_id);
        }
        if ($this->isColumnModified(ProductImageTableMap::COL_IMAGE_ID)) {
            $criteria->add(ProductImageTableMap::COL_IMAGE_ID, $this->image_id);
        }
    ...
    public function hashCode()
    {
        $validPk = null !== $this->getProductId() &&
            null !== $this->getImageId();

        $validPrimaryKeyFKs = 2;
        $primaryKeyFKs = [];

        //relation product_image_fk_0f5ed8 to table product
        ...
        //relation product_image_fk_810cd0 to table image
        if ($this->aImage && $hash = spl_object_hash($this->aImage)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }
    ...
    {
        return (null === $this->getProductId()) && (null === $this->getImageId());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ProductImage (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ProductImage Clone of current object.
     * @throws PropelException
     */
    ...
    public function getImage(ConnectionInterface $con = null)
    {
        if ($this->aImage === null && ($this->image_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aImage->addProductImages($this);
             */
        }
    ...
    public function clear()
    {
        ...
        if (null !== $this->aImage) {
            $this->aImage->removeProductImage($this);
        }
